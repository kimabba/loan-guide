...
export function ProductsPage() {
  ...
  useEffect(() => {
    ...
    if (selectedProductTypes.length) params.set("types", selectedProductTypes.join(","));
    if (selectedCompanies.length) params.set("companies", selectedCompanies.join(","));
    if (viewMode !== "all") params.set("view", viewMode);
    setSearchParams(params, { replace: true });
  }, [search, selectedCategories, selectedProductTypes, selectedCompanies, viewMode, setSearchParams]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [products, viewMode, favorites, search, selectedCategories, selectedProductTypes, selectedCompanies]);
      ...
      <div className="border-b bg-card">
        <div className="mx-auto max-w-7xl px-4 py-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-bold text-foreground">대출 상품 탐색</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {filteredProducts.length}개 상품 {products.length !== filteredProducts.length && `(전체 ${products.length}개)`}
              </p>
            </div>
          ...
          >
            ...
            <span className="text-muted-foreground">
              {selectedCategories.length + selectedProductTypes.length + selectedCompanies.length > 0
                ? `${selectedCategories.length + selectedProductTypes.length + selectedCompanies.length}개 선택`
                : "선택 없음"}
            </span>
          </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mb-2 text-lg font-medium text-foreground">
              {viewMode === "favorites" ? "즐겨찾기한 상품이 없습니다" : "검색 결과가 없습니다"}
            </h3>
